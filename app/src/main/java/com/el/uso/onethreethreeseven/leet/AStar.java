...
public class AStar {
    ...
    private int hvCost;
    private int mDiagonalCost;
    private Node[][] mSearchArea;
    private PriorityQueue<Node> mOpenList;
    ...
    public AStar(int rows, int cols, Node initialNode, Node finalNode, int hvCost, int diagonalCost) {
        this.hvCost = hvCost;
        this.mDiagonalCost = diagonalCost;
        setInitialNode(initialNode);
        setFinalNode(finalNode);
        this.mSearchArea = new Node[rows][cols];
        this.mOpenList = new PriorityQueue<>(new Comparator<Node>() {
            @Override
        ...
        setNodes();
        this.mClosedList = new ArrayList<Node>();
    }
    ...
    private void setNodes() {
        for (int i = 0; i < mSearchArea.length; i++) {
            for (int j = 0; j < mSearchArea[0].length; j++) {
                Node node = new Node(i, j);
                node.calculateHeuristic(getFinalNode());
                this.mSearchArea[i][j] = node;
            }
        }
    }
    ...
    public List<Node> findPath() {
        mOpenList.add(mInitialNode);
        while (!isEmpty(mOpenList)) {
            Node currentNode = mOpenList.poll();
    ...
    private boolean isFinalNode(Node currentNode) {
        return currentNode.equals(mFinalNode);
    }
    ...
    private void setBlock(int row, int col) {
        this.mSearchArea[row][col].setBlock(true);
    }
